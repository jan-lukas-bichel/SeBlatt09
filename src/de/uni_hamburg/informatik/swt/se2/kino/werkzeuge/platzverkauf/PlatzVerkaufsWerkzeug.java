...
public class PlatzVerkaufsWerkzeug {
	...
	private void aktualisierePreisanzeige(Set<Platz> plaetze) {
		_ausgewaehlterGesamtbetrag = Geldbetrag.select(0, 0);
		if (istVerkaufenMoeglich(plaetze)) {
			Geldbetrag preis = _vorstellung.getPreisFuerPlaetze(plaetze);
			_ui.getPreisLabel().setText("Gesamtpreis: " + preis.konvertiereString());
			_ausgewaehlterGesamtbetrag = preis;
		} else if (istStornierenMoeglich(plaetze)) {
			Geldbetrag preis = _vorstellung.getPreisFuerPlaetze(plaetze);
			_ui.getPreisLabel().setText("Gesamtstorno: " + preis.konvertiereString());
